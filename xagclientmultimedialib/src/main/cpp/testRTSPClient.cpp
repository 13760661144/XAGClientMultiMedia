...
Java_cn_xag_xagclientmultimefialib_jniutils_Live555Rtsp_Live555Rtsp(JNIEnv *env, jclass type,
                                                                    jstring path_) {
    if (!isrunning) {
        ...
        _methodid1 = env->GetMethodID(type, "onNativeInfo", "(Ljava/lang/String;)V");
        if (_methodid1 == NULL) {
...
Java_cn_xag_xagclientmultimefialib_RTSPClient_stop(JNIEnv *env, jclass type) {
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
    ...
    if (strcmp(fSubsession.mediumName(), "video") == 0) {
        ...
        if (fReceiveBuffer[0] == 0x00 && fReceiveBuffer[1] == 0x00 && fReceiveBuffer[2] == 0x00 &&
            fReceiveBuffer[3] == 0x01) {
            runningFlag=0;
            if (_methodid2 == NULL) {
                if (_clazz == NULL) {
                    //com.rtsp.poe.rtsplive555
                    _clazz = _env->FindClass("cn/xag/xagclientmultimefialib/jniutils/Live555Rtsp");
                    if (_clazz == NULL) {
                        tmp = "can't find class: \"com/xag/live555/RTSPClient\"";
                        infoCallBack(charToJstring(_env, tmp.c_str()));
                    }
                }
                if (_obj == NULL && _clazz != NULL) {
            ...
            memcpy(buffer, fReceiveBuffer, frameSize);
