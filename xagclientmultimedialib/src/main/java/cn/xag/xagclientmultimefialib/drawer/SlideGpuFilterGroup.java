...
import cn.xag.xagclientmultimefialib.helper.MagicFilterFactory;
import cn.xag.xagclientmultimefialib.helper.MagicFilterType;
...
public class SlideGpuFilterGroup {
    ...
    public SlideGpuFilterGroup(Context context ,int screenWidth ) {
    ...
    private GPUImageFilter getFilter(int index) {
        GPUImageFilter filter = MagicFilterFactory.initFilters(types[index],mContext);
        if (filter == null) {
    ...
    public void onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                locked = true;
                downX = -1;
                if (offset > mScreenWidth / 3) {
                    scroller.startScroll(offset, 0, mScreenWidth - offset, 0, 100 * (1 - offset /mScreenWidth));
                    needSwitch = true;
                } else {
